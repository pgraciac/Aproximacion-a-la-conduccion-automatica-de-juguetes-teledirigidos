...
def follow_path():
    global actual_orientation, spin_right_second, distance_second, path_spins
    ...
    def pathing(last_point, estado_robot, last_orientation, f, n_errores = 0):
        global angulo_closest_point, closest_point, actual_orientation, path_spins
        vision.mostrar_frame("not_orientation")
        print("pathing")
        f.write("pathing\n")
        ...
        if vision.point != last_point: ##ver si cambia dos al menos
            ...
            if actual_distance > 80:
                ...
                f.write("esta en la linea\n")
                good_orientation = angulo_closest_point
                last_path_point = closest_point
            else:
            ...
            #ESTIMAR EL GIRO ANTES EN FUNCIÓN DE DONDE GIRA EL CAMINO
            distance_next_spin = distance_two_points(vision.point, path_spins[0][0])
            print(f"la distancia al proximo giro es: {distance_next_spin}, el proximo giro es: {path_spins[0]}")
            f.write(f"la distancia al proximo giro es: {distance_next_spin}, el proximo giro es: {path_spins[0]}\n")

            if distance_next_spin < 40:
                print("calculando good orientation anticipadamente")
                f.write("calculando good orientation anticipadamente\n")
                good_orientation = path_spins[0][1]
                error_orientation = estimacion_orientacion - good_orientation
                error_orientation = (error_orientation + 180) % 360 - 180
                print("girando anticipadamemte")
                f.write("girando anticipadamente\n")
            if distance_next_spin < 10:
                print("pasando el giro")
                f.write("pasando el giro\n")
                path_spins.pop(0)
    ...
    def begin_path():
        global up_button, right_button, left_button, actual_orientation, robot_pathing, path_spins
        ...
        time.sleep(1)
        print(f"path_spins: {path_spins}")
        if len(path_spins) <= 0:
            path_spins = []
            path_orientation = average_orientation(vision.path, 0, 5)
            for i in range(0, len(vision.path) - 5):
                actual_path_orientation = average_orientation(vision.path, i, 5)
                error = (path_orientation - actual_path_orientation + 180) % 360 - 180
                if abs(error) > 60:
                    print(average_orientation(vision.path, i + 1, 5, True))
                    print(vision.path[i+1], "\n------------------------------------\n")
                    path_orientation = average_orientation(vision.path, i + 1, 5)
                    path_spins.append((vision.path[i+1], path_orientation))
